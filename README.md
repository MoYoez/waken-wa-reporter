...
| 平台 | 状态 |
| --- | --- |
| Windows / macOS | 支持全部当前特性，包括后台实时同步、托盘驻留、开机自启动、平台自检、手动活动上报、灵感发布与 Discord Rich Presence 同步 |
| Linux | 支持全部当前特性；Wayland 已适配 GNOME / KDE，媒体信息采集依赖 `playerctl` |
| Android | 支持除 iOS 外的全部当前特性，提供手机与平板自适应 UI，并支持常驻通知、手动活动上报、灵感发布与相关平台能力 |
| iOS | 暂未完整适配，当前不作为完整支持平台提供 |
